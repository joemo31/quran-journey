...
const { sendEnrollmentNotification } = require('../config/whatsapp');
...
const create = async (req, res, next) => {
  try {
    ...
    // 1. Send WhatsApp to admin phone (server-side, fully automatic)
...
const getAll = async (req, res, next) => {
  try {
    ...
    const countRes = await query(
      `SELECT COUNT(*) FROM form_submissions WHERE ${where}`,
      params.slice(0, -2)
    );
    const result = await query(
